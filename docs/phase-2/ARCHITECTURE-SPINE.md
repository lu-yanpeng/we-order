...
**一处已记录的偏离**：Phase 1 AD-1 含「API 层不持有运行时状态」，而 AD-14 让会话凭证与续期状态住在 `api/` 内——字面上相抵。ly 裁示：Phase 2 对小程序只做一小块改动，**接受此偏离**，Phase 3 正式对接时一并修正。这是**有期限的例外，不是对本条继承约束的重新解释**。**已于 Phase 3 关闭**：会话移入 `core/session`，`api/` 恢复不持有运行时状态（见 Phase 3 spine AD-2/AD-4 与本文修订记录）。
...
- **Rule:** `orders` 与 `order_items` 不存在任何面向客户端的写策略，写权限不授予 `anon`/`authenticated`；订单只能经服务端函数创建。归属取自会话身份，客户端传入的任何用户标识被忽略。不变量：数据库里不存在「金额由客户端决定」的行。**Phase 3 收紧**：`create_order` 收回客户端 EXECUTE，成为只被服务端接缝（`create_order_for_user`，只授 `service_role`）调用的内核；写路径仍唯一（见 Phase 3 spine AD-12）。
...
- **Rule:** 订单列表与详情经服务端函数读取，函数内先推进调用者到点的订单、再返回结果。`orders` 的本人 SELECT 策略保留作纵深防御，但客户端契约是走函数。**权限模型**：面向用户态写操作的 RPC（下单、催单、确认取杯、订单读取）一律 `SECURITY DEFINER` 并设置 `search_path = ''`，自身按 AD-3 携带归属谓词；这些 RPC 不附带任何额外的表权限，客户端除发布密钥外不获得直接表访问。**Phase 3 补充**：订单读取契约仍是服务端函数；订阅（publication + RLS 过滤的变更事件）是对本条「不直读表」的显式例外（见 Phase 3 spine AD-9/AD-13）。
...
- **Rule:**
  ...
  - **发号与迁移分离**：取杯号在**下单时**分配、一经分配不可变；推进只做状态迁移，不携带发号参数（见 AD-7）——「有号」不表示已到点，「能否取餐」由状态表达。
  - **触发**：「何时该推进」（下单后 15 秒、催单提前到 3 秒、Phase 4 的商家点击）来自**同一处配置**，不写死在判断里，可整体替换而不触碰机制。用户不点确认时，订单还会按门店配置的等待时长**超时自动完成**——自动完成时刻在进入「待取餐」时写入订单，之后改配置不影响已出的单；它与用户确认取杯共用 `pickup→completed` 这一处迁移实现，只是触发源不同。
  - **作用域**：读时推进只作用于**调用者自己**到点的订单；周期兜底作用于**全部**到点订单。两者调用同一实现；自动完成的读时判定与周期扫描沿用同一分工（读取订单时即时完成调用者超时的订单、周期扫描兜底全部超时订单）。
...
- **Binds:** FR-P2-11
- **Prevents:** 并发重号；出现「有单无号」的脏数据；中途换号
- **Rule:** 取杯号在**下单时**分配，与订单创建由**同一条语句**完成——客户端始终能凭号询问进度，不必等到出餐。唯一域是「门店 + 门店本地自然日」，唯一性由该域上的唯一约束保证；发号日期取下单时刻的门店本地自然日，不随后续状态变化重算。分配通过计数器原子递增（`insert ... on conflict do update ... returning`），因此**允许跳号，不允许重号**——跳号只是数字不连续，重号才是事故。不用「查最大值加一」。取杯号一经分配**不可变**（推进与催单都不写它）。
...
- **Rule:** 会话的持有、持久化、过期判断、续期与失败回退全部封在 `api/` 内，对 composable 与页面不可见（判据：上层代码不出现 token 一词；**请求头构造随 Phase 3 AD-2 迁至 `core/transport`**）。`api/` 不向上层暴露「未登录」状态。**登录时机已由 Phase 3 AD-4 显式修订为「启动即静默登录」**（原「登录为惰性、启动不强制」不再有效）。会话不进入 Pinia。**续期实现必须单飞**——同一时刻只允许一个续期在飞，其余请求等待并复用其结果。Phase 2 只实现登录一半，此规则约束 Phase 3 的对接实现。
...
- **Rule:** 测试用 pgTAP，以 `supabase test db` 一条命令运行。模拟身份使用平台当前的 claim 注入方式（历史写法为 `set local role authenticated` 与 `set local request.jwt.claim.sub`，平台已引入 JSON 形式的 claim GUC）——**写法本身不写入契约，以实现时的官方文档为准，契约是测试结果**。不引入第三方 helper。至少覆盖：三类边界（他人数据不可见 / 不可写 / 未认证不可读）、AD-3 的两处归属谓词等价性、金额重算、非法迁移被拒、重复推进幂等、幂等键并发去重。测试必须能在重建后的空库上直接通过。**Phase 3 调整**：权限收紧（Phase 3 AD-12）后，既有用例改经 `create_order_for_user` 调用（`service_role`、用户 UUID 由 fixture 提供、移除 claim 注入），静态权限断言按新授权反转，另加反向（直呼被拒）与正向（身份注入生效）断言；verify 脚本同批改造（8 文件 9 处）。
...
- **Rule:** 以下共享形状各只有一处定义，且定义在数据库侧（类型或映射函数），不在各调用点重复：
  ...
  - **订单对外形状**：列表与详情的字段名、空值规则（取杯号恒有值）、时间与金额格式完全一致，由同一映射产出。
...
| 订单号 | 18 位纯数字（门店本地时间 `YYYYMMDDHHmmss` + 4 位随机尾号），服务端生成，全局唯一由唯一约束加冲突重试保证 |
| 取杯号 | 字母前缀 + 四位数字（`A-0008`），同一前缀用尽后进入下一个字母（`A-9999 → B-0001`）；**下单时分配、恒有值**、一经分配不可变；允许跳号，不允许重号；「能否取餐」由状态表达 |
...
| 定时调度 | Supabase Cron（pg_cron，支持秒级周期 1–59 seconds；Phase 3 起扫描周期 3s，同名替换 `order-sweep`） |
...
| 项目 | 推迟原因 / 再访条件 |
| --- | --- |
| ~~Phase 2 对 Phase 1 AD-1 的已知偏离：会话与续期状态住在 `api/` 内~~ | **已关闭（Phase 3）**：会话移入 `core/session`，`api/` 恢复不持有运行时状态 |
...
- 2026-09-25：Phase 3 架构回写（spine 定稿）。① AD-14 惰性登录条款被 Phase 3 AD-4 显式修订为「启动即静默登录」，请求头构造判据迁至 `core/transport`；② 偏离记录段与 Deferred 对应行关闭（会话移入 `core/session`）；③ AD-5 补订阅显式例外（publication + RLS 变更事件，非行读取）；④ AD-2 补 `create_order` 权限收紧（服务端内核 + 只授 `service_role` 的包装接缝）；⑤ AD-19 测试基线调用面调整（pgTAP 62 处 + verify 脚本 9 处）；⑥ Stack 定时调度行改为秒级（1–59s），Phase 3 起扫描周期 15s→3s 同名替换。
