...
- **Rule:** 小程序侧在 Phase 2 只接登录链路（取一次性凭证 → 调登录接口拿会话 → 本地持久化；生成与兑换都在服务端完成）与一个最小验证入口。目录与订单继续读 Mock；不为 Phase 3 预建正式对接层。本 spine 描述的服务端契约是 **Phase 3 的目标态**，不等于 Phase 2 已接线。
...
| 取杯号 | 字母前缀 + 两位数字（`A-08`）；未进入「待取餐」时为空值；允许跳号，不允许重号 |
