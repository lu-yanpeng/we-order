...
- **Rule:** 结算栏组件位于分包中，使用微信小程序占位组件（`componentPlaceholder`）机制实现按需加载：(a) 页面中声明占位组件，分包未下载时占位组件不渲染任何内容（空节点）；(b) 进入应用时若购物车已有数据，立即触发分包下载，结算栏替换占位组件并从底部滑入；(c) 首次进入且购物车为空时不触发分包下载，占位组件保持空渲染；(d) 首次加购时触发分包下载，结算栏替换占位组件并从底部滑入；(e) 一旦分包下载完成，结算栏永久可见、不销毁；(f) 若尚未加载，再来一单同样触发分包下载，并自动展开购物车面板（展开指令在组件挂载后执行）。来源 PRD FR-5、FR-14。
