...
- 离线且确定：外部调用通过注入假 fetch 模拟，不需要网络与真实凭证
- 类别断言以数据库类型 `login_error_code` 为准；文案不是契约，不断言措辞

| 文件 | 覆盖 |
| --- | --- |
| `wechat-login/wechat-login.test.ts` | wechat-login：微信错误码 → `login_error_code` 映射（含未知码）、请求形状（官方地址与参数）、入参校验、连不上微信/非 JSON/HTTP 5xx、AppSecret 不外泄 |
