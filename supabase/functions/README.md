...
- **生成**：官方 Admin API `generate_link`（`type=magiclink`，邮箱用由 openid 派生的占位邮箱 `wx-<openid>@wechat.local`）。它只生成令牌、不发邮件；SDK 归一化后的 `data.properties.hashed_token` 就是下文的一次性令牌。
...
测试放 `functions/tests/<函数名>/`（Supabase 官方推荐布局，函数目录只留参与部署的代码），覆盖：微信错误码 → 类别映射（含无法真实构造的 40226 / 45011 / 45009 / -1）、请求体与请求方法边界、缺少服务端配置、响应不泄露密钥与微信原文、类别 → 状态码、派生 id/邮箱的确定性、首登建用户、再登复用、并发撞车（422 email_exists / 500 视为可能撞车，由 claim 与外键兜底）分支、令牌生成（调用形状、成功与失败分支）。
...
- 平台调用走官方 `@supabase/supabase-js`（service role 客户端只在 `handler.ts` 里构造一处；自定义 fetch 仅用于测试注入）
