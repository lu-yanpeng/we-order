...
- 2026-09-24 真机两身份（FR-P2-14 / FR-P2-15 / FR-P2-18、UJ-P2-1；证据形式：实现方式说明 + 人工验证记录）：
  - 环境与前提：本地 Docker 栈在跑；两个真实微信号（该小程序的开发/体验成员）在真机上跑开发版；**同一局域网直连**——手机与电脑在同一热点网段，把电脑地址的 54321 转发到本地栈、入站防火墙放行 TCP 54321（转发方式随开发机平台而异）；开始验证前先用手机浏览器访问 `http://<电脑地址>:54321/auth/v1/health` 拿到 GoTrue 健康信息；`mp/.env.local` 指向该地址并重新编译生效。
