 ...
 * - 错误归一化为 AuthError：code 是稳定类别，message 只给人看
 * - 两类服务端错误载荷都兼容：边缘函数的 { code, message }、
 *   平台 auth 的 { code, error_code, msg }
 */
import { supabasePublishableKey, supabaseUrl } from './config'

/** 错误类别：暂时按字符串承载，Story 2.6 收口到数据库生成的类别枚举 */
export type AuthErrorCode = string

export class AuthError extends Error {
  /** 稳定类别（服务端错误类别，或客户端判定的 network_unreachable） */
  ...
  return new Promise<T>((resolve, reject) => {
    uni.request({
      ...
      success: (result) => {
        ...
        reject(new AuthError(code, message, result.statusCode))
      },
      fail: (error) => {
